...
namespace CUETools.Codecs.FLAKE
{
	...
	public class FlakeReader: IAudioSource
	{
		...
		unsafe void decode_frame_header(BitReader bitreader, FlacFrame frame)
		{
			...
			// Resolve the frame sample rate, including custom encodings and
			// code 0, which inherits the value from STREAMINFO.
			uint sr;
			if (sr_code0 == 0)
				sr = (uint)PCM.SampleRate;
			else if (sr_code0 <= 11)
				sr = (uint)Flake.flac_samplerates[sr_code0];
			else
			{
				switch (sr_code0)
				{
			...
			if (sr != PCM.SampleRate)
				throw new Exception("frame sample rate differs from STREAMINFO");
