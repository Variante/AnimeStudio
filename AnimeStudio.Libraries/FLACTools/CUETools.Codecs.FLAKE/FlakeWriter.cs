...
new int[] { // 30
        ...
        unsafe void output_frame_header(FlacFrame frame, BitWriter bitwriter)
        {
            ...
            // Custom sample rate. The payload width is selected by the frame
            // header code, not by the numeric value itself.
            if (sr_code0 == 12)
            {
                bitwriter.writebits(8, sr_code1);
            }
            else if (sr_code0 == 13 || sr_code0 == 14)
            {
        ...
        void set_samplerate_code()
        {
            int sampleRate = Settings.PCM.SampleRate;
            if (sampleRate <= 0 || sampleRate > 655350)
                throw new ArgumentOutOfRangeException(
                    nameof(Settings.PCM.SampleRate),
                    sampleRate,
                    "FLAC sample rate must be between 1 and 655350 Hz."
                );

            sr_code1 = 0;

            // Prefer the predefined frame-header codes when possible.
            for (int i = 1; i < 12; i++)
            {
                if (sampleRate == Flake.flac_samplerates[i])
                {
                    sr_code0 = i;
                    return;
                }
            ...
            // FLAC frame-header codes 12-14 carry a custom rate as kHz,
            // Hz, or tens of Hz respectively. Code 0 inherits the rate from
            // STREAMINFO and covers the remaining legal values.
            if (sampleRate % 1000 == 0 && sampleRate / 1000 <= byte.MaxValue)
            {
                sr_code0 = 12;
                sr_code1 = sampleRate / 1000;
            }
            else if (sampleRate <= ushort.MaxValue)
            {
                sr_code0 = 13;
                sr_code1 = sampleRate;
            }
            else if (sampleRate % 10 == 0 && sampleRate / 10 <= ushort.MaxValue)
            {
                sr_code0 = 14;
                sr_code1 = sampleRate / 10;
            }
            else
            {
                sr_code0 = 0;
